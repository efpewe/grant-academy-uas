  ...
  useEffect(() => {
    ...
    const activateAccount = async () => {
      try {
        ...
        setStatus("success");
        setMessage(
          "Akun Anda berhasil diaktifkan! Silakan login untuk memulai.",
        );
      } catch (err: any) {
        setStatus("error");
        setMessage(
          err.response?.data?.message ||
            "Gagal mengaktifkan akun. Token mungkin sudah kedaluwarsa.",
        );
      }
    };
  ...
  return (
    ...
    >
      <div className="text-center py-6">
        ...
        <h3
          className={`text-lg font-bold font-lexend mb-2 
        ...
        <p className="text-gray-600 font-lexend mb-8 px-4">{message}</p>
        ...
        {status !== "loading" && (
          <div className="space-y-3">
            ...
            {status === "error" && (
              <Link
                to="/register"
                className="block text-sm text-primary hover:underline mt-4 font-lexend"
              >
                Daftar ulang?
              </Link>
            )}
