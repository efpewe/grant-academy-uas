...
export default function CreateCourse() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      navigate('/dashboard/courses'); // Redirect ke list course
    } catch (error) {
      console.error(error);
      ...
      <div className="max-w-3xl mx-auto pb-10">
        ...
        <div className="flex items-center justify-between mb-8">
            <div>
                ...
                <p className="text-gray-500 text-sm mt-1">Isi detail di bawah untuk mempublikasikan materi baru.</p>
            </div>
            <button 
                onClick={() => navigate(-1)}
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 space-y-6">
            ...
            <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                    <div className={`w-2 h-2 rounded-full ${formData.price === 0 ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                    <p className="text-xs text-gray-500">
                        {formData.price === 0 ? "Kursus ini akan diset sebagai GRATIS." : "Harga dalam Rupiah."}
                    </p>
                </div>
            </div>
