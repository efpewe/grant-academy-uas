...
export default function AddLesson() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      // LOGIC PEMBERSIHAN DATA SEBELUM KIRIM
      const payload = {
          ...formData,
          // Jika tipe Text, paksa videoUrl jadi kosong agar backend tidak bingung
  ...
  return (
    <DashboardLayout>
      <div className="max-w-3xl mx-auto bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        <h1 className="text-2xl font-bold mb-2 font-lexend">Tambah Materi Baru</h1>
        <p className="text-gray-500 text-sm mb-6">Pilih jenis materi yang ingin Anda sampaikan.</p>
        
        {/* --- PILIHAN TIPE MATERI (TAB SWITCHER) --- */}
        <div className="flex bg-gray-100 p-1 rounded-xl mb-6">
            <button
                type="button"
                onClick={() => setLessonType('video')}
                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${
                    lessonType === 'video' ? 'bg-white text-primary shadow-sm' : 'text-gray-500 hover:text-gray-700'
                }`}
            >
            ...
            <button
                ...
                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${
                    lessonType === 'text' ? 'bg-white text-primary shadow-sm' : 'text-gray-500 hover:text-gray-700'
                }`}
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
           ...
           <div>
              ...
              <input 
                ...
                placeholder={lessonType === 'video' ? "Contoh: Intro to React Hooks" : "Contoh: Panduan Instalasi Node.js"}
              />
           </div>

           {/* FIELD KHUSUS VIDEO (Hanya muncul jika type video) */}
           {lessonType === 'video' && (
               <div className="animate-fade-in-down">
                  ...
                  <input 
                    ...
                    onChange={handleChange}
                    required={lessonType === 'video'} // Wajib diisi hanya jika mode video
                  ...
                  />
                  <p className="text-xs text-gray-400 mt-1">Pastikan video tidak di-private.</p>
               </div>
           )}

           {/* KONTEN UTAMA (Berubah fungsi tergantung tipe) */}
           <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">
                  {lessonType === 'video' ? 'Deskripsi Video' : 'Isi Artikel / Materi Bacaan'}
              </label>
              <textarea 
                name="content" 
                ...
                onChange={handleChange}
                required
                // Jika tipe Text, area tulis lebih besar (15 baris)
           ...
           <div className="flex gap-4 pt-4 border-t border-gray-100">
              ...
              >
                 {loading ? 'Menyimpan...' : (
                    <>
                        <span>💾</span> Simpan Materi {lessonType === 'text' ? 'Bacaan' : 'Video'}
                    </>
                 )}
              </button>
           </div>
